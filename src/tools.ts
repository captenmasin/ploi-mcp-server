...
export const DEFAULT_USER_AGENT = `ploi-mcp-server/${VERSION}`;

type HttpMethod = "GET" | "POST" | "PATCH" | "DELETE";
type QueryParams = Record<string, string | number | boolean | undefined>;
type JsonObject = Record<string, unknown>;

export interface PloiClientConfig {
  apiToken: string;
  baseUrl?: string;
  userAgent?: string;
}

export class PloiApiError extends Error {
  constructor(
    message: string,
...
export class PloiClient {
  ...
  constructor(config: PloiClientConfig) {
...
/**
 * Register every Ploi tool on the given MCP server. Shared by the stdio entrypoint
 * (src/index.ts) and the Cloudflare Worker entrypoint (src/worker.ts).
 */
export function registerPloiTools(server: McpServer, client: PloiClient): void {
  server.registerTool(
    "ploi_list_servers",
    {
